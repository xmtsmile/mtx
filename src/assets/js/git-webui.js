...
webui.showError = function (message) {
  var $error = $("#git-webui-error-modal");
  
  if ($error.length == 0) {
    $error = $('<div class="modal fade" id="git-webui-error-modal" tabindex="-1" role="dialog">' +
      '<div class="modal-dialog" style="width: 450px;">' +
      '<div class="modal-content">' +
      '<div class="modal-header" style="padding: 3px 10px;background-color: rgb(152, 0, 0);color: #fff;">' +
      '<button type="button" class="close" data-dismiss="modal" style="padding-right: 2px;padding-top: 4px;"><span>&times;</span><span class="sr-only">Close</span></button>' +
      '<h4 class="modal-title">Error</h4></div>' +
      '<pre class="modal-body alert alert-danger" style="padding: 8px; border: 0; min-height: 64px; background-color: #fff;"></pre>' +
      '</div></div></div>').appendTo($('#git-webui'));
  }
  
  $error.find(".alert").text(message);
  $error.modal('show');
};
...
webui.showWarning = function (message) {
  ...
  messageBox.empty();
  $('<div class="alert alert-warning alert-dismissible" role="alert" style="padding: 6px;">' +
    '<button type="button" class="close" data-dismiss="alert" style="top: -3px; right: 0;">' +
...
webui.git = function (cmd, arg1, arg2) {
  ...
  var project = JSON.parse(sessionStorage.getItem("projectInfo"));
  ...
  $.ajax({
    ...
    dataType: "text",
    headers: {
      "X-User-Repo": project.projectName,
    ...
    if (xhr.status == 200) {
      ...
      if (rcode == 0) {
        ...
        if (message.length > 0) {
          // there is decode error while showing the Chinese ...
          // but somewhere else do not has the problem
          // so here, for convenience, just ignore the warning message
          console.log(message);
          // webui.showWarning(message);
        }
        $("#error-modal").find(".alert").text("");
      } else {
        // console.log(message);
...
webui.SideBarView = function (mainView) {
  ...
  self.addPopup = function (section, title, id, refs, tooltips) {
    var popup = $('<div class="modal fade" id="' + id + '" role="dialog">' +
      '<div class="modal-dialog" style="width: 450px;">' +
      '<div class="modal-content">' +
      '<div class="modal-header" style="padding: 3px 10px;background-color: rgb(51,51,51);color: #fff;">' +
      '<button type="button" class="close" style="padding-right: 2px;padding-top: 4px;" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>' +
      '<h4 class="modal-title">' + title + '</h4>' +
      '</div>' +
      '<div class="modal-body" style="padding: 0; overflow-x: scroll; height: 288px;"><div class="list-group"></div></div>' +
      '</div>' +
      '</div>' +
      '</div>')[0];
    ...
    refs.forEach(function (ref) {
      ...
      $(co).click(function (event) {
        var tmpRefName = $(event.target).parent()[0].refName;
        // console.log(tmpRefName);
        event.stopPropagation();
        webui.git("checkout " + tmpRefName, function (data) {
          $("#sidebar-local-branches > ul", self.element).empty();
          $("#local-branches-popup").remove();
          $("div.modal-backdrop.fade.in").remove();
          self.fetchSection($("#sidebar-local-branches", self.element)[0], "Local Branches", "local-branches", "branch", true);
        });
      });
      ...
      if (tooltips) {
        var tpContent = "";
        if (link.refName == "master" || link.refName == "develop") {
          tpContent = "The branched is protected, DO NOT work on it";
        } else {
          $.post("/mtx/development/task/developer", {
            branchName: $(".sidebar-ref.branch-current").text()
          }, function (data) {
  ...
  self.fetchSection = function (section, title, id, gitCommand, tooltips) {
    webui.git(gitCommand, function (data) {
      ...
      if (refs.length > 0) {
        ...
        for (var i = 0; i < refs.length && i < maxRefsCount; ++i) {
          ...
          if (tooltips) {
            var tpContent = "";
            if (li.refName == "master" || li.refName == "develop") {
              tpContent = "The branched is protected, DO NOT work on it";
            } else {
              $.post("/mtx/development/task/developer", {
                branchName: $(".sidebar-ref.branch-current").text()
        ...
        if (refs.length > maxRefsCount) {
          li = $('<li class="sidebar-more">More ...</li>').appendTo(ul);
          var popup = self.addPopup(section, title, id + "-popup", refs, tooltips);
          li.click(function () {
            $(popup).modal();
          });
        }
  ...
  self.update = function (refresh) {
    if (refresh) {
      $("#sidebar-local-branches > ul", self.element).remove();
      $("#sidebar-remote-branches > ul", self.element).remove();
      $("#sidebar-tags > ul", self.element).remove();
    }
    
    self.fetchSection($("#sidebar-local-branches", self.element)[0], "Local Branches", "local-branches", "branch", true);
    self.fetchSection($("#sidebar-remote-branches", self.element)[0], "Remote Branches", "remote-branches", "branch --remotes", false);
    self.fetchSection($("#sidebar-tags", self.element)[0], "Tags", "tags", "tag", false);
  };
  
  self.mainView = mainView;
  self.element = $('<div id="sidebar">' +
    '<a href="#"><img id="sidebar-logo" src="/assets/images/git-logo.png"></a>' +
...
webui.LogView = function (historyView) {
    ...
    webui.git("log --date-order --pretty=raw --decorate=full --max-count=" + (maxCount + 1) + " " + self.nextRef + " --", function (data) {
      ...
      while (true) {
        ...
        if (count < maxCount) {
          ...
          entry.element.setAttribute("style", "height:auto;");
          if (!currentSelection) {
            entry.select();
          }
        } else {
  ...
  self.updateGraph = function (startAt) {
    ...
    for (i = startAt; i < content.children.length; ++i) {
  ...
  function Entry(logView, data) {
    ...
    self.createElement = function () {
      self.element = $('<a class="log-entry list-group-item">' +
        ...
        '</header>' +
        '<p class="list-group-item-text" style="margin-top: 4px;"></p>' +
        '<p style="margin: 0; margin-top: 4px;>' +
        '<span class="log-entry-date">' + self.author.date.toLocaleString() + '</span>' +
        '<span style="margin-left: 40px; min-width: 30px;">' + self.commit + '</span></p></a>')[0];
      $('<a target="_blank" style="margin-right: 10px;" href="mailto:' + self.author.email + '">' + self.author.name + '</a>').appendTo($("h6", self.element));
      ...
      if (self.refs) {
        ...
        self.refs.forEach(function (ref) {
          ...
          $('<span>&nbsp;</span><span class="label label-' + reftype + '" style="padding: 2px;">' + ref + '</span>').insertAfter(entryName);
        });
      }
  ...
  $.contextMenu({
    ...
    items: {
      "new": {
        ...
        callback: function (key, opt) {
          console.log("Commit Id is : " + opt.$trigger[0].model.commit);
          console.log("Tree Id is : " + opt.$trigger[0].model.tree);
        }
      },
      "sep3": "---------",
      "deploy": {
      ...
      "branch": {
        ...
        "items": {
          "resolve": {
            name: "Resolve ...",
            callback: function (key, opt) {
              console.log("Current branch is : " + $(".sidebar-ref.branch-current").text());
            }
          },
          // "sep2": "---------",
          "verify": {
    ...
    events: {
      show: function (options) {
        if (!options.$trigger.hasClass("active")) {
          options.$trigger.addClass("x-context-menu-active");
        }
      },
      hide: function (options) {
        options.$trigger.removeClass("x-context-menu-active");
      }
    }
  });
  
  $(".data-title").attr("data-menutitle", "Operations");
  $(".context-menu-root ul.context-menu-list").addClass("data-title").attr("data-menutitle", "Dependencies");
};
...
webui.DiffView = function (sideBySide, hunkSelectionAllowed, parent) {
  ...
  self.handleClick = function (event) {
    ...
    if (cmd == "+" || cmd == "-") {
      ...
      for (elt = lineElt.nextElementSibling; elt; elt = elt.nextElementSibling) {
...
webui.TreeView = function () {
  ...
  self.showBlob = function () {
    self.createBreadcrumb();
    $(self.element.lastElementChild).remove();
    $('<div id="tree-view-blob-content"></div>').appendTo(self.element);
    
    var editor = ace.edit("tree-view-blob-content");
    editor.getSession().setTabSize(4);
    editor.setReadOnly(true);
    
    webui.git("cat-file -p " + self.stack[self.stack.length - 1].object, function (data) {
...
webui.CommitExplorerView = function (mainView, diff) {
  ...
  self.buildDiffSections = function () {
...
webui.FileListView = function (commitExplorerView, files) {
  ...
  self.viewScrolled = function (event) {
    ...
    if (event.target == rightScrollView) {
      current = rightScrollView;
      other = leftScrollView;
    } else {
      current = leftScrollView;
      other = rightScrollView;
    }
    other.scrollTop = current.scrollTop;
  };
...
webui.ChangedFilesView = function (workspaceView, type, label) {
  ...
  self.process = function () {
    ...
    if (files.length != 0) {
      cmd = type == "working-copy" ? "add" : "reset";
      webui.git(cmd + " -- " + files, function () {
        if (rmFiles.length != 0) {
          webui.git("rm -- " + rmFiles, function () {
      ...
      cmd = type == "working-copy" ? "rm" : "reset";
      webui.git(cmd + " -- " + rmFiles, function () {
        workspaceView.update(type == "working-copy" ? "stage" : "unstage");
      });
    }
  };
  ...
  self.cancel = function () {
    prevScrollTop = fileListContainer.scrollTop;
    var files = self.getFileList(undefined, "?");
    var newFiles = self.getFileList("?");
    
    if (files.length != 0) {
      webui.git("checkout -- " + files, function () {
        if (newFiles.length != 0) {
          webui.git("clean -qf " + newFiles, function () {
      ...
      webui.git("clean -qf -- " + files, function () {
        workspaceView.update("stage");
      });
    }
  };
  ...
  var buttons;
  if (type == "working-copy") {
    buttons = [{name: "Stage", callback: self.process}, {name: "Reset", callback: self.cancel}];
  } else {
    buttons = [{name: "Unstage", callback: self.process}];
  }
...
webui.CommitMessageView = function (workspaceView) {
  ...
  self.onCommit = function () {
    if (workspaceView.stagingAreaView.filesCount == 0) {
      ...
      var flag_push = $("#cb_push", self.element).prop("checked");
      var flag_finish = $("#cb_finish", self.element).prop("checked");
      
      var cmd = "commit ";
      cmd += "--file=-";
      webui.git(cmd, textArea.value, function () {
        textArea.value = "";
        workspaceView.update("stage");
        
        if (flag_push) {
          webui.git("push -u --all", function () {
            window.mainui.sideBarView.update(true);
          });
        }
        
        if (flag_finish) {
          // notify the DBA's work of task (branch) is finished
          $.post("/mtx/development/task/finish", {
            branchName: $(".sidebar-ref.branch-current").text()
          }, function (data) {
            data = JSON.parse(data);
            if (data.code != 0) {
              webui.showError("can not tag current task as finished");
            }
          }).fail(function () {
            webui.showError("can not tag current task as finished");
          });
  ...
  self.onFinishTask = function () {
    if ($(this).prop("checked")) {
      textArea.value = "##DB_READY## " + textArea.value;
    } else {
      textArea.value = textArea.value.replace("##DB_READY## ", "");
    }
  };
  ...
  self.element = $('<div id="commit-message-view" class="panel panel-default">' +
    '<div class="panel-heading">' +
    '<h5>Message</h5>' +
    '<div class="btn-group btn-group-sm form-inline">' +
    '<div class="checkbox" style="padding: 4px;margin-right: 8px;">' +
    '<label><input style=" margin-right: 4px;top: 2px;" type="checkbox" name="push" id="cb_push" checked>Push The Commit</label>' +
    '</div>' +
    '<div class="checkbox" style="padding: 4px;margin-right: 8px;">' +
    '<label><input style=" margin-right: 4px;top: 2px;" type="checkbox" name="finish" id="cb_finish">Finish The Task</label>' +
    '</div>' +
    '<button type="button" style="float:right;" class="btn btn-default commit-message-commit">Commit</button>' +
    '</div>' +
    '</div>' +
    '<textarea></textarea>' +
    '</div>')[0];
  ...
  $("#cb_finish", self.element).change(self.onFinishTask);
};
